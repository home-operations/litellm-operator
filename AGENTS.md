...
- **Idempotent.** Reconcilers, code generators (`mise run generate`), and CLI
  ...
  affects correctness, only latency. For this repo specifically: `Reconcile`
  must converge from any starting state without depending on event ordering.
...
- **Unit tested**, table-driven via `t.Run` subtests. Stdlib `testing` is the
  ...
  mixing in a second framework: this repo's `internal/controller` and
  `internal/webhook` suites are Ginkgo/Gomega, keep new controller/webhook
  tests in that style, and use Go table tests (`testify`) for pure logic
  like `internal/litellmclient`. `go test -race` is the floor (`mise run
...
- **`github.com/spf13/pflag`, only when the app has a real CLI surface**:
  ...
  This repo is the exception in practice: `cmd/main.go` is a kubebuilder
  scaffold, so it keeps the scaffold's stdlib `flag` +
  `opts.BindFlags(flag.CommandLine)` wiring rather than converting generated
  operator boilerplate to pflag.

## Project layout

`cmd/main.go` is the entrypoint; everything else lives under `internal/`
unless another repo needs to import it as a library (`github.com/home-operations/flate`
is imported directly by `downflate` and `konflate`, which is why it isn't
under `internal/` there; this repo has no such export surface). Keep
`main.go` to wiring: parse config/flags, build the manager, register
controllers/webhooks, run, translate the top-level error into an exit code.
Business logic belongs in `internal/controller`, `internal/webhook`, etc.,
not in `main`.
...
parameter: `Reconcile` and webhook handlers already get one from
controller-runtime, propagate it, don't build a new `context.Background()`
partway down the call stack. Prefer `golang.org/x/sync/errgroup` over raw
...
This repo also has `mise run manifests` (CRDs/RBAC/webhook config codegen),
`generate` (deepcopy etc.), `test-integration`, `test-e2e`, `helm-*`, and
`run`; check `mise tasks` before adding one that might already exist under a
different name.
...
(`generate-check`/`manifests` drift); run both locally before pushing. Lint
rules live in `.golangci.yml`; read it instead of trusting a restated list
here, since the two can drift.
...
Prometheus metrics via `github.com/prometheus/client_golang` (or
controller-runtime's built-in metrics registry, already wired via
`--metrics-bind-address` in this repo) on a separate port from health
probes.
